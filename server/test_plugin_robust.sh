#!/bin/bash
# Mem0 Plugin Comprehensive Test Suite - Robust Version
...
API_RESPONSE=$(curl -s -X POST $BASE_URL/admin/keys \
    ...
    -d '{"agent_id": "test_agent", "description": "Plugin test"}')
...
echo -e "${BLUE}Test 1.1: Create Memory${NC}"
START=$(date +%s.%N)
...
if [ "$HTTP_CODE" -eq 401 ] || [ "$HTTP_CODE" -eq 403 ]; then
    record_result "Server: Invalid API Key" "PASS" "Correctly rejected with $HTTP_CODE"
else
...
if [ "$HTTP_CODE" -eq 400 ] || [ "$HTTP_CODE" -eq 422 ]; then
    record_result "Server: Missing Parameters" "PASS" "Correctly rejected with $HTTP_CODE"
else
    record_result "Server: Missing Parameters" "FAIL" "Unexpected HTTP code: $HTTP_CODE"
fi

# Test 3.3: Search non-existent memory
echo -e "\n${BLUE}Test 3.3: Query Non-Existent Memory${NC}"
RESULT=$(curl -s -X POST $BASE_URL/search \
    -H 'Content-Type: application/json' \
    -H "X-API-Key: $API_KEY" \
    -d '{"query": "nonexistent_12345", "user_id": "test_user", "agent_id": "test_agent"}')
COUNT=$(echo "$RESULT" | jq '.results | length')
if [ "$COUNT" -eq 0 ]; then
    record_result "Server: Query Non-Existent Memory" "PASS" "Correctly returned empty results"
else
    record_result "Server: Query Non-Existent Memory" "FAIL" "Unexpected: $COUNT results"
fi
...
HAS_AGENT_1_IN_1=$(echo "$RESULT_1" | jq -r '.results[].memory' 2>/dev/null | grep -c "Agent 1" || echo 0)
HAS_AGENT_2_IN_1=$(echo "$RESULT_1" | jq -r '.results[].memory' 2>/dev/null | grep -c "Agent 2" || echo 0)
HAS_AGENT_2_IN_2=$(echo "$RESULT_2" | jq -r '.results[].memory' 2>/dev/null | grep -c "Agent 2" || echo 0)
