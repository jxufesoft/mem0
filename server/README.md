...
```bash
curl -X GET "${SERVER_URL}/memories/mem_abc123" \
  -H "X-API-Key: ${API_KEY}"
```
...
```bash
curl -X PUT "${SERVER_URL}/memories/mem_abc123" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: ${API_KEY}" \
  -d '{
...
```bash
curl -X GET "${SERVER_URL}/memories/mem_abc123/history" \
  -H "X-API-Key: ${API_KEY}"
```
...
```bash
curl -X DELETE "${SERVER_URL}/memories/mem_abc123" \
  -H "X-API-Key: ${API_KEY}"
```
